package com.kalsym.whatsapp.service.filter;

import com.kalsym.whatsapp.service.model.HttpReponse;
import com.kalsym.whatsapp.service.model.MySQLUserDetails;
import com.kalsym.whatsapp.service.model.Auth;
...
public class SessionRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        Logger.application.info(Logger.pattern, WhatsappServiceApplication.VERSION, "------------- " + request.getMethod() + " " + logprefix + "-------------", "", "");

        final String authHeader = request.getHeader("Authorization");
        Logger.application.warn(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "Authorization: " + authHeader, "");
        ...
        if (null != authHeader && authHeader.startsWith("Bearer ")) {
            ...
            Logger.application.warn(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "token: " + accessToken, "");
            Logger.application.warn(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "token length: " + accessToken.length(), "");
            tokenPresent = true;
        } else {
            Logger.application.warn(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "token does not begin with Bearer String", "");
        }
            ...
            try {
                ...
                if (null != expiryTime && null != username) {
                    ...
                    try {
                        ...
                        Logger.application.warn(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "error calculating time to session expiry", "");
                    }
                    Logger.application.info(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "time to session expiry: " + diff + "ms", "");
                    if (0 < diff) {
                        authorized = true;
                        ...
                        Logger.application.warn(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "session expired", "");
                        //response.setStatus(HttpStatus.UNAUTHORIZED);
                        response.getWriter().append("Session expired");
                    }
                }
            } catch (IOException | IllegalArgumentException | RestClientException e) {
                Logger.application.error(Logger.pattern, WhatsappServiceApplication.VERSION, logprefix, "Exception processing session ", "", e);
